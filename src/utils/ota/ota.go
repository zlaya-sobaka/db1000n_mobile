...
func WatchUpdates(cfg *Config) {
	...
	if !cfg.skipUpdateCheckOnStart {
		...
		mobilelogger.Infof("Version update on startup is skipped, next update check scheduled in %v",
...
func runUpdate(doRestartOnUpdate bool) {
	mobilelogger.Infof("Running a check for a newer version...")

	isUpdateFound, newVersion, changeLog, err := doAutoUpdate()

	switch {
	case err != nil:
		mobilelogger.Infof("Auto-update failed: %v", err)

		return
	case !isUpdateFound:
		mobilelogger.Infof("We are running the latest version, OK!")

		return
	}

	mobilelogger.Infof("Newer version of the application is found [version=%s]", newVersion)
	mobilelogger.Infof("What's new:\n%s", changeLog)

	if !doRestartOnUpdate {
		mobilelogger.Infof("Auto restart is disabled, restart the application manually to apply changes!")

		return
	}

	mobilelogger.Infof("Auto restart is enabled, restarting the application to run a new version")

	if err = restart("-skip-update-check-on-start"); err != nil {
		mobilelogger.Infof("Failed to restart the application after the update to the new version: %v", err)
		mobilelogger.Infof("Restart the application manually to apply changes!")
	}
}
